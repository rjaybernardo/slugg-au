...
jQuery(".selected_data").on("click", function(){
  jQuery(this).closest(".product-variant").find("ul").toggleClass("variant_open");
...
theme.custom_stickyCart = function(){
  ...
  jQuery(".select-variant").on("click", function(){
    jQuery(this).closest(".product-variant").find("ul").removeClass("variant_open");
    jQuery(this).closest(".product-variant").find(".selected_data").removeClass("active");
    jQuery(".select-variant").removeClass("selected");
    jQuery(this).addClass("selected");
    var variantTitle = jQuery(this).find(".variant_title").attr("title");
    var variantImage = jQuery(this).find(".variant_colors").attr("variant_image");
  ...
  jQuery(".product-block-variant-picker .variant-input").on("click", function(){
...
theme.countdown_timer = function(){
  if(jQuery('.current_date_time_main').length > 0){
    var afterDayDelivery = parseInt(jQuery('.current_date_time_main').attr('data-after-day'));
  
    var originalDateString = jQuery('.current_date_time_main').attr('data-date-time');
    ...
    dataTimeCountdown = jQuery('.current_date_time_main').attr('data-time-countdown').split(',');
  
    targetTime.setHours(dataTimeCountdown[0], dataTimeCountdown[1], dataTimeCountdown[2]);
    ...
    function updateCounter() {
      ...
      var formattedTime = ('0' + hours).slice(-2) + 'h and ' + ('0' + minutes).slice(-2) + ' miniutes';
      // console.log(formattedTime2);
  
      var expectedDeliveryText = '<span>Free metro delivery expected on <b>' + convertedDateStr + '</b> if you order in the next <b>' + formattedTime + '</b></span>';
...
jQuery(window).scroll(function(){ 
  if(jQuery("body.template-product").length > 0){
    if(jQuery('.sticky_cart.product_full_info_sticky').length == 0){
      var addToCartOffsetTop = jQuery('.product-block-container .shopify-product-form .add-to-cart-btn').offset().top + 100;
